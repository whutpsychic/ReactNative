...
const renderListItem = ({
	...
	return (
		...
		>
			...
			<p className="detail">
				<span>发生时间：{obj.date1}</span>
				<span>发布时间：{obj.date2}</span>
			</p>
		</div>
	);
};
...
class Detail extends React.Component {
	...
	render() {
		...
		const { name, unit, type, date1, date2, files = [] } = data;
		return show ? (
			...
			<div className="right-drawer-container">
				...
				<div className="main-container">
					<ul>
						...
						<li>
							<label>发生时间</label>
							<DatePicker ref="date" placeholder="请选择时间" clearable />
						</li>
					</ul>
