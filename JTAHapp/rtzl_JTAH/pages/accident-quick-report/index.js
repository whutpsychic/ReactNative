...
class Default extends React.Component {
  ...
  onReceive = (event) => {
    ...
    // 点击更多
      ...
      const {files, date1, date2, unit, name, type} = receivedData;
      this.postMessage({
        etype: 'data',
        detail: {
          files,
  ...
  query = (page = 0, conditions = {}) => {
    ...
    api.getAccidentQuickReport({page, ps, ...conditions}).then((res) => {
      console.log(res);
      ...
      // 成功
        ...
        let dataArr = data.list.map((item) => {
          return {
            date1: item.createrTime,
            date2: item.accidentTime,
