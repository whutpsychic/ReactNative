...
class Default extends React.Component {
  ...
  onReceive = (event) => {
    ...
    // 当改变查询条件的时候
      ...
      const {type, type2, number, name} = receivedData;
      let condition = {
        name,
  ...
  query = (page = 0, conditions = {}, bool) => {
    ...
    api.getLawStandardList({page, ps, ...conditions}).then((res) => {
      console.log(res);
      ...
      // 成功
        ...
        let dataArr = data.list.map((item) => {
          return {
            name: item.name,
            type1: transTypeToText(item.classify),
            type2: transTypeToText2(item.category),
            date1: item.publishTime,
            date2: item.effectiveTime,
