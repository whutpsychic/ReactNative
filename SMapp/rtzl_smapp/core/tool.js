...
//格式转换 YYYY-MM-DDTHH:MM:SS+MM:SS => YYYY-MM-DD HH:MM:SS
tool.transTimer = str => {
	console.log(str);

	let prevTreg = /[0-9]+-[0-9]+-[0-9]+T/;
	let nextTreg = /T[0-9]+:[0-9]+:[0-9]+/;

	let prevT = prevTreg.exec(str)[0].replace('T', '');
	let nextT = nextTreg.exec(str)[0].replace('T', ' ');
	return prevT + nextT;
};
// ------------------------业务工具----------------
